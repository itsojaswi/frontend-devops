...
const WorkoutList = () => {
  ...
  useEffect(() => {
    const fetchWorkouts = async () => {
      try {
        const response = await axios.get(`https://127.0.0.1:4000/api/workouts`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://127.0.0.1:4000/api/workouts/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const name = await axios.patch(`https://127.0.0.1:4000/api/workouts/${editingWorkout._id}`, editingWorkout);
      console.log(name);
