...
const WorkoutList = () => {
  ...
  useEffect(() => {
    const fetchWorkouts = async () => {
      try {
        const response = await axios.get(`https://54.179.187.198:4000/api/workouts`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://54.179.187.198:4000/api/workouts/${id}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const name = await axios.patch(`https://54.179.187.198:4000/api/workouts/${editingWorkout._id}`, editingWorkout);
      console.log(name);
